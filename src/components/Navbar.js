...
const Navbar = (props) => {
    ...
    return (
        <>
            <div className="navbar p-0 container-fluid bg-warning">
                <div className={`row w-100 justify-content-between m-auto align-items-center `}>
                    ...
                    <div className="col-md-8 col-12 order-md-2 order-3 d-flex justify-content-between align-items-center">
                        <ul id="ul" className={`nav-${props.ul} flex-column flex-md-row w-100 d-flex justify-content-end my-auto align-items-center`}>
                            ...
                            <li className="nav-item my-2">
                                <Link className={`nav-items mr-md-5 d-${localStorage.getItem('token')? 'block' : 'none'} ${location.pathname === '/mynotes' ? 'active' : ''}`} to="/mynotes" id='hanged'>My Notes</Link>
                            </li>
                            <li className="nav-item my-2">
                                <Link className={`nav-items mr-md-5 ${location.pathname === '/about' ? 'active' : ''}`} to="/about">About</Link>
                            </li>
                            {!localStorage.getItem('token')?<div>
                                <Link className="btn btn-outline-primary mx-2 my-md-0 my-1" to="/login">Login</Link>
                                <Link className="btn btn-outline-primary mx-2 my-0 my-1" to="/signup" >Signup</Link>
                            </div>
                                : <Link className='btn btn-outline-danger mx-2 my-1' to='/login' onClick={handleLogout} >Logout</Link>}
                        </ul>
                    </div>
